    ...
    if (assets) {
      ...
      if (links && links.length > 0) {
        ...
        return links.map((link) => {
          ...
          const file = link?.asset?.fields?.file[locale];
          const fields = link?.asset?.fields;
          if (file?.contentType?.indexOf('image') < 0) {
            return link.asMarkdown;
          }
          const attributes = [];
          file?.url && attributes.push(`src="${file.url}"`);
          file?.details?.image?.height && attributes.push(`height="${file.details.image.height}"`);
          file?.details?.image?.width && attributes.push(`width="${file.details.image.width}"`);
          fields?.title && fields.title[locale] && attributes.push(`alt="${fields.title[locale]}"`);
          fields?.description && fields.description[locale] && attributes.push(`title="${fields.description[locale]}"`);
